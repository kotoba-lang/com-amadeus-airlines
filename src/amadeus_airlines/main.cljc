(ns amadeus_airlines.main
  "Kotodama WASM entrypoint for the Amadeus_airlines clean-room actor (L4) — Clojure port.
  ...
  facts (`amadeus_airlines.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amadeus_airlines")
(def tier "L4")
...
(defn page-limit
  ...
  src/amadeus_airlines/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn emit-facts
  "EAVT facts for one record: {\"amadeus_airlines.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amadeus_airlines-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amadeus_airlines-compat/ready)
